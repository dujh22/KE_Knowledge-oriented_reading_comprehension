...
class ODQA:
    ...
    def predict(self,
        ...
        tfidf_retrieval_output = self.tfidf_retriever.get_abstract_tfidf('DEMO', question, self.args)
        ...
        reader_output = [{'question':s['question'],'answer': answer[s['q_id']]} for s in graph_retrieval_output]
...
def main():
    ...
    print()
    while True:
        with open('KoRC-H_valid.txt') as f:
            questions=f.read()
        ...
        for question in questions:
            try:
                tfidf_retrieval_output, graph_retriever_output, reader_output = odqa.predict(question)
            ...
            #json.dump(graph_retriever_output, f)
            #print() 
                data = json.dumps(reader_output, indent=2)            
                with open('predict_KoRC_H_valid.json','a', newline='\n') as fp:
                    fp.write(data)
                print(reader_output)
                print()

            except:
                data=json.dumps([{"question": "{}".format(question),"answer": "None"}])
                with open('predict_KoRC_H_valid.json','a', newline='\n') as fp:
                    fp.write(data)
                print()                
